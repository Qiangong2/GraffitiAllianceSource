...
import okhttp3.internal.p110ws.RealWebSocket;
...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:17:0x0082 A[Catch:{ IOException -> 0x009d }] */
    ...
    private Request createTunnel(int readTimeout, int writeTimeout, Request tunnelRequest, HttpUrl url) throws IOException {
        ...
        do {
            ...
            switch (response.code()) {
                ...
                case GCloudVoiceErrno.GCLOUD_VOICE_STOP_PLAY_FILE:
        ...
        } while (!"close".equalsIgnoreCase(response.header(HttpConstants.Header.CONNECTION)));
        return tunnelRequest;
    }

    private Request createTunnelRequest() {
        return new Request.Builder().url(this.route.address().url()).header(HttpConstants.Header.HOST, Util.hostHeader(this.route.address().url(), true)).header("Proxy-Connection", "Keep-Alive").header(HttpConstants.Header.USER_AGENT, Version.userAgent()).build();
    }
    ...
    public RealWebSocket.Streams newWebSocketStreams(final StreamAllocation streamAllocation) {
        return new RealWebSocket.Streams(true, this.source, this.sink) {
            /* class okhttp3.internal.connection.RealConnection.C21281 */
