...
public final class CacheInterceptor implements Interceptor {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        return response.newBuilder().body(new RealResponseBody(response.headers(), Okio.buffer(new Source() {
            /* class okhttp3.internal.cache.CacheInterceptor.C21231 */
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) {
        ...
        for (int i2 = 0; i2 < size2; i2++) {
            ...
            if (!HttpConstants.Header.CONTENT_LENGTH.equalsIgnoreCase(fieldName2) && isEndToEnd(fieldName2)) {
    ...
    static boolean isEndToEnd(String fieldName) {
        if (HttpConstants.Header.CONNECTION.equalsIgnoreCase(fieldName) || "Keep-Alive".equalsIgnoreCase(fieldName) || "Proxy-Authenticate".equalsIgnoreCase(fieldName) || "Proxy-Authorization".equalsIgnoreCase(fieldName) || "TE".equalsIgnoreCase(fieldName) || "Trailers".equalsIgnoreCase(fieldName) || HttpConstants.Header.TRANSFER_ENCODING.equalsIgnoreCase(fieldName) || "Upgrade".equalsIgnoreCase(fieldName)) {
            return false;
        }
        return true;
    }
