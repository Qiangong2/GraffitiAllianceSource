...
public final class Cookie {
    ...
    private static boolean pathMatch(HttpUrl url, String path2) {
        ...
        if (!urlPath.startsWith(path2) || (!path2.endsWith(MqttTopic.TOPIC_LEVEL_SEPARATOR) && urlPath.charAt(path2.length()) != '/')) {
            return false;
        }
        return true;
    }
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        if (path2 == null || !path2.startsWith(MqttTopic.TOPIC_LEVEL_SEPARATOR)) {
            String encodedPath = url.encodedPath();
            int lastSlash = encodedPath.lastIndexOf(47);
            path2 = lastSlash != 0 ? encodedPath.substring(0, lastSlash) : MqttTopic.TOPIC_LEVEL_SEPARATOR;
        }
