...
public class DatabaseFieldConfigLoader {
        ...
        while (true) {
            try {
                ...
                if (line != null && !line.equals(CONFIG_FILE_END_MARKER)) {
                    if (line.length() != 0 && !line.startsWith(MqttTopic.MULTI_LEVEL_WILDCARD) && !line.equals(CONFIG_FILE_START_MARKER)) {
    ...
    public static void writeConfig(BufferedWriter writer, DatabaseFieldConfig config, String tableName) throws IOException {
        ...
        if (config.getUnknownEnumValue() != null) {
            writer.append(FIELD_NAME_UNKNOWN_ENUM_VALUE).append('=').append((CharSequence) config.getUnknownEnumValue().getClass().getName()).append(MqttTopic.MULTI_LEVEL_WILDCARD).append((CharSequence) config.getUnknownEnumValue().name());
            writer.newLine();
        }
        ...
        if (foreignColumnName != null) {
            writer.append(FOREIGN_COLUMN_NAME).append('=').append((CharSequence) foreignColumnName);
            writer.newLine();
        }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r13v0, resolved type: com.j256.ormlite.field.DatabaseFieldConfig */
    ...
    private static void readField(DatabaseFieldConfig config, String field, String value) {
        if (field.equals(FIELD_NAME_FIELD_NAME)) {
            ...
            String[] parts = value.split(MqttTopic.MULTI_LEVEL_WILDCARD, -2);
            if (parts.length != 2) {
