...
public class MappedUpdateId<T, ID> extends BaseMappedStatement<T, ID> {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r14v0, resolved type: com.j256.ormlite.dao.ObjectCache */
    ...
    public int execute(DatabaseConnection databaseConnection, T data, ID newId, ObjectCache objectCache) throws SQLException {
        ...
        try {
            ...
            if (rowC > 0) {
                if (!(objectCache == 0 || (updateId = objectCache.updateId(this.clazz, this.idField.extractJavaFieldValue(data), newId)) == null || updateId == data)) {
            ...
            throw SqlExceptionUtil.create("Unable to run update-id stmt on object " + ((Object) data) + ": " + this.statement, e);
        }
    }
