...
public class StatementExecutor<T, ID> implements GenericRowMapper<String[]> {
    ...
    public long queryForLong(DatabaseConnection databaseConnection, String query, String[] arguments) throws SQLException {
        ...
        try {
            stmt = databaseConnection.compileStatement(query, StatementBuilder.StatementType.SELECT, noFieldTypes);
    ...
    public GenericRawResults<String[]> queryRaw(ConnectionSource connectionSource, String query, String[] arguments, ObjectCache objectCache) throws SQLException {
        ...
        CompiledStatement compiledStatement = null;
        try {
            compiledStatement = connection.compileStatement(query, StatementBuilder.StatementType.SELECT, noFieldTypes);
    ...
    public <UO> GenericRawResults<UO> queryRaw(ConnectionSource connectionSource, String query, RawRowMapper<UO> rowMapper, String[] arguments, ObjectCache objectCache) throws SQLException {
        ...
        try {
            compiledStatement = connection.compileStatement(query, StatementBuilder.StatementType.SELECT, noFieldTypes);
            assignStatementArguments(compiledStatement, arguments);
            RawResultsImpl<UO> rawResults = new RawResultsImpl<>(connectionSource, connection, query, String[].class, compiledStatement, new UserObjectRowMapper(rowMapper, this), objectCache);
            compiledStatement = null;
            connection = null;
            return rawResults;
        } finally {
    ...
    public int updateRaw(DatabaseConnection connection, String statement, String[] arguments) throws SQLException {
        ...
        CompiledStatement compiledStatement = connection.compileStatement(statement, StatementBuilder.StatementType.UPDATE, noFieldTypes);
        try {
            assignStatementArguments(compiledStatement, arguments);
    ...
    public int executeRaw(DatabaseConnection connection, String statement, String[] arguments) throws SQLException {
        ...
        CompiledStatement compiledStatement = connection.compileStatement(statement, StatementBuilder.StatementType.EXECUTE, noFieldTypes);
        try {
            assignStatementArguments(compiledStatement, arguments);
    ...
    public <CT> CT callBatchTasks(DatabaseConnection connection, boolean saved, Callable<CT> callable) throws SQLException {
        ...
        try {
            ...
            try {
                ...
                throw SqlExceptionUtil.create("Batch tasks callable threw lib_non-SQL exception", e2);
            }
        } finally {
    ...
    public boolean ifExists(DatabaseConnection connection, ID id) throws SQLException {
        if (this.ifExistsQuery == null) {
            ...
            qb.selectRaw("COUNT(*)");
            qb.where().mo6812eq(this.tableInfo.getIdField().getColumnName(), new SelectArg());
    ...
    public static class UserObjectRowMapper<UO> implements GenericRowMapper<UO> {
        ...
        public UserObjectRowMapper(RawRowMapper<UO> mapper2, GenericRowMapper<String[]> stringMapper) {
            this.mapper = mapper2;
