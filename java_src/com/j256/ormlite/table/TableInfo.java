...
public class TableInfo<T, ID> {
    ...
    public TableInfo(ConnectionSource connectionSource, BaseDaoImpl<T, ID> baseDaoImpl2, Class<T> dataClass2) throws SQLException {
        this(connectionSource.getDatabaseType(), baseDaoImpl2, DatabaseTableConfig.fromClass(connectionSource, dataClass2));
    }

    public TableInfo(DatabaseType databaseType, BaseDaoImpl<T, ID> baseDaoImpl2, DatabaseTableConfig<T> tableConfig) throws SQLException {
        ...
        for (FieldType fieldType : fieldTypeArr) {
            if (fieldType.isId() || fieldType.isGeneratedId() || fieldType.isGeneratedIdSequence()) {
                if (findIdFieldType != null) {
                    throw new SQLException("More than 1 idField configured for class " + this.dataClass + " (" + findIdFieldType + MiPushClient.ACCEPT_TIME_SEPARATOR + fieldType + ")");
                }
    ...
    public FieldType getFieldTypeByColumnName(String columnName) {
        ...
        FieldType fieldType2 = this.fieldNameMap.get(columnName);
        if (fieldType2 != null) {
            return fieldType2;
        }
        FieldType[] fieldTypeArr2 = this.fieldTypes;
        for (FieldType fieldType22 : fieldTypeArr2) {
